...
public class ParentChildInnerHitContextBuilderExt extends InnerHitContextBuilder {
    private final String typeName;
    private final boolean fetchChildInnerHits;

    ParentChildInnerHitContextBuilderExt(String typeName, boolean fetchChildInnerHits, QueryBuilder query, InnerHitBuilder innerHitBuilder, Map<String, InnerHitContextBuilder> children) {
    ...
    static final class JoinFieldInnerHitSubContext extends InnerHitsContext.InnerHitSubContext {
        ...
        public TopDocs[] topDocs(SearchHit[] hits) throws IOException {
            ...
            boolean onlyProperHits = getName().equalsIgnoreCase("proper_hits");
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                builder.add(new BooleanClause(new BooleanQuery.Builder()
                    ...
                    .add(joinFieldMapper.fieldType().termQuery(typeName, qsc), BooleanClause.Occur.FILTER).build(), BooleanClause.Occur.SHOULD));

                String parentId = getSortedDocValue(parentIdFieldMapper.name(), context, hit.docId());
                if (!Strings.isNullOrEmpty(parentId)) {
                    builder.add(new BooleanClause(parentIdFieldMapper.fieldType().termQuery(parentId, qsc), BooleanClause.Occur.SHOULD));
                }
                ...
                if (size() == 0) {
                    if (!onlyProperHits) {
                        ...
                        context.searcher().search(q, totalHitCountCollector);
        ...
        public TopDocs[] topDocs(SearchHit[] hits) throws IOException {
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                BooleanQuery q = new BooleanQuery.Builder()
                    ...
                    .add(documentMapper.typeFilter(context.getQueryShardContext()), BooleanClause.Occur.FILTER).build();
                Weight weight = context.searcher().createNormalizedWeight(q, false);
                if (size() == 0) {
                    TotalHitCountCollector totalHitCountCollector = new TotalHitCountCollector();
                    for (LeafReaderContext ctx : context.searcher().getIndexReader().leaves()) {
